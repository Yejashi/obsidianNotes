...
$$f(n)=Θ(g(n))$$if and only if$f(n)=O(g(n))$ and f(n) = \Theta(g(n)) \quad \text{if and only if} \quad f(n) = O(g(n)) \text{ and } f(n) = \Omega(g(n))f(n)=Θ(g(n))if and only iff(n)=O(g(n)) and f(n)=Ω(g(n))
