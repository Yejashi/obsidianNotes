...
```
Besides defining a member for each token, we added two additional values: `eoi` and `anonymous`. `eoi` stands for end of input and is returned when all characters of the input are processed.
